// src/data/passive.js
export const MONSTER_SKILLS = [
  // ==========================================
  // 🌿 Map 1: Serene Meadow
  // ==========================================
  {
  id: 'Bug Carapace',
  monster: 'แมลงตัวน้อย',
  name: 'เปลือกแมลงสะท้อน', // เปลี่ยนชื่อให้ดูเข้ากับความสามารถใหม่
  description: 'สะท้อนความเสียหาย 3% กลับไปยังศัตรู',
  element: 'earth',
  icon: '🪞', // เปลี่ยนไอคอนเป็นกระจกหรือโล่หนามเพื่อให้สื่อถึงการสะท้อน
  rarity: 'Common',
  bonusAtk: 0,
  bonusDef: 0, // เอาโบนัสป้องกันออก (หรือจะเก็บไว้สัก 1-2 ก็ได้แล้วแต่ชอบครับ)
  bonusMaxHp: 0,
  reflectChance: 1.0, // เพิ่มค่านี้เพื่อให้รู้ว่าสะท้อนแน่นอน (100%)
  reflectDamage: 0.05 // เพิ่มค่า 0.03 (คิดเป็น 3%)
},
  {
    id: 'Caterpillar Silk',
    monster: 'หนอนน้อย',
    name: 'ใยไหมเหนียวหนึบ',
    description: 'เพิ่มพลังป้องกันและพลังชีวิตเล็กน้อย',
    element: 'nature',
    icon: '🧶',
    rarity: 'Common',
    bonusAtk: 0,
    bonusDef: 2,
    bonusMaxHp: 10
  },
  {
    id: 'Slime Recovery',
    monster: 'สไลม์ทุ่งหญ้า',
    name: 'ฟื้นฟูเยลลี่',
    description: 'ร่างกายยืดหยุ่นเพิ่มพลังชีวิตสูงสุด',
    element: 'water',
    icon: '🧪',
    rarity: 'Common',
    bonusAtk: 0,
    bonusDef: 1,
    bonusMaxHp: 25
  },
  {
    id: 'Grasshopper Jump',
    monster: 'ตั๊กแตน',
    name: 'ก้าวกระโดดพริ้วไหว',
    description: 'เพิ่มพลังโจมตีจากการจู่โจมที่รวดเร็ว',
    element: 'wind',
    icon: '🦗',
    rarity: 'Uncommon',
    bonusAtk: 4,
    bonusDef: 0,
    bonusMaxHp: 0
  },
  {
    id: 'Aura', 
    monster: 'ราชินีผึ้งทองคำ',
    name: 'ออร่าแห่งราชินี',
    description: 'บารมีแห่งผึ้งทองคำ เพิ่มพลังโจมตีและโชคลาภ',
    element: 'holy',
    icon: '👑',
    rarity: 'Legendary',
    bonusAtk: 12,
    bonusDef: 5,
    bonusMaxHp: 50
  },

  // ==========================================
  // 🌲 Map 2: Emerald Valley
  // ==========================================
  {
    id: 'Rock Skin',
    monster: 'แมลงหินจิ๋ว',
    name: 'ผิวหนังศิลา',
    description: 'ร่างกายแข็งแกร่งดั่งหินผา เพิ่มป้องกันอย่างมาก',
    element: 'earth',
    icon: '🪨',
    rarity: 'Uncommon',
    bonusAtk: 0,
    bonusDef: 8,
    bonusMaxHp: 15
  },
  {
    id: 'Wolf Hunter',
    monster: 'หมาป่าหุบเขา',
    name: 'สัญชาตญาณนักล่า',
    description: 'ความบ้าคลั่งของหมาป่า เพิ่มพลังโจมตีอย่างสูง',
    element: 'fire',
    icon: '💢',
    rarity: 'Uncommon',
    bonusAtk: 15,
    bonusDef: 0,
    bonusMaxHp: 0
  },
  {
    id: 'Regeneration',
    monster: 'สไลม์มรกต',
    name: 'มรกตคืนชีพ',
    description: 'พลังแห่งพงไพรช่วยฟื้นฟูร่างกายตลอดเวลา',
    element: 'nature',
    icon: '🌱',
    rarity: 'Uncommon',
    bonusAtk: 0,
    bonusDef: 4,
    bonusMaxHp: 100
  },
  {
    id: 'Frost Bite',
    monster: 'หมาป่าหิมะ',
    name: 'คมเขี้ยวเยือกแข็ง',
    description: 'พลังโจมตีธาตุน้ำแข็งที่รุนแรงและเยือกเย็น',
    element: 'ice',
    icon: '❄️',
    rarity: 'Rare',
    bonusAtk: 25,
    bonusDef: 5,
    bonusMaxHp: 0
  },
  {
    id: 'Royal Aura',
    monster: 'ราชาสไลม์',
    name: 'ออร่าแห่งราชา',
    description: 'บารมีของผู้นำสไลม์ เพิ่มพลังทุกด้าน',
    element: 'holy',
    icon: '👑',
    rarity: 'Rare',
    bonusAtk: 15,
    bonusDef: 15,
    bonusMaxHp: 150
  },
  {
    id: 'Diamond Armor',
    monster: 'แมลงเพชร',
    name: 'เกราะเพชรเจียระไน',
    description: 'เกราะสะท้อนแสงที่ไม่มีวันแตกสลาย',
    element: 'holy',
    icon: '💎',
    rarity: 'Epic',
    bonusAtk: 5,
    bonusDef: 20,
    bonusMaxHp: 50
  },
  {
    id: 'Emerald Blessing',
    monster: 'สไลม์หยกนพเก้า',
    name: 'พรแห่งหยกนพเก้า',
    description: 'พลังศักดิ์สิทธิ์ที่เพิ่มขีดจำกัดของชีวิต',
    element: 'holy',
    icon: '✨',
    rarity: 'Epic',
    bonusAtk: 10,
    bonusDef: 10,
    bonusMaxHp: 250
  },
  {
    id: 'Golden Touch',
    monster: 'มหาจักรพรรดิสไลม์ทองคำ',
    name: 'สัมผัสทองคำ',
    description: 'พลังระดับตำนานที่เปลี่ยนทุกอย่างเป็นทองคำและความแข็งแกร่ง',
    element: 'legend',
    icon: '🔱',
    rarity: 'Legendary',
    bonusAtk: 50,
    bonusDef: 30,
    bonusMaxHp: 500
  }
];