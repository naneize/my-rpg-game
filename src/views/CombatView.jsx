...
export default function CombatView({ 
  ...
  // ... (handleFinalizeCombat เหมือนเดิม)

  return (
    <div 
      className={`relative w-full h-[calc(100vh-140px)] md:h-full flex flex-col overflow-y-auto no-scrollbar text-white transition-colors duration-1000 ${
        isWorldBoss ? 'bg-black' : 'bg-slate-950'
      }`}
      style={{
        backgroundImage: isWorldBoss 
    ...
    >
      {/* 🛡️ ครอบด้วย BossFrame และจัดการ Layout ภายในใหม่ */}
      <div className="flex-1 flex flex-col w-full max-w-xl mx-auto px-2 py-4">
        ...
        <BossFrame 
          ...
          lootResult={lootResult}
        >
          {/* 👾 1. โซนมอนสเตอร์ (ส่วนบน) */}
          <div className="relative flex flex-col justify-center min-h-[250px] mb-4">
            <div className="absolute inset-0 pointer-events-none z-[110] flex items-center justify-center">
              {skillTexts && skillTexts.map((skill) => (
                <SkillFloatingText key={skill.id} name={skill.name} />
              ))}
          ...
          {/* ⚔️ 2. โซนปุ่มกด (ส่วนกลาง) */}
          <div className="space-y-2 mb-6">
            <button 
              onClick={onAttack} 
              disabled={isInputLocked} 
              className={`w-full py-4 text-white font-black rounded-3xl shadow-2xl flex items-center justify-center gap-3 text-lg uppercase italic transition-all
                ${isInputLocked 
            ...
            {!lootResult && (
              <button 
                onClick={onFlee} 
                disabled={isInputLocked} 
                className={`w-full py-3 bg-slate-800 text-white font-black rounded-3xl flex items-center justify-center gap-3 text-sm uppercase italic active:scale-95`}
              >
                <Footprints size={18} /> <span>ถอยไปตั้งหลัก!</span> 
              </button>
            )}
          ...
          {/* 💖 3. โซนสเตตัสผู้เล่น (ส่วนล่าง - กลับมาแล้ว!) */}
      ...
      {/* Modals & Damage Numbers (เหมือนเดิม) */}
      <VictoryLootModal lootResult={lootResult} monster={monster} onFinalize={handleFinalizeCombat} stats={player} />
      <div className="absolute inset-0 pointer-events-none z-50 overflow-hidden">
        {damageTexts?.map((dmg) => <DamageNumber key={dmg.id} value={dmg.value} type={dmg.type} />)}
      </div>
