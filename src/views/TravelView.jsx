...
import { ChevronRight, Skull, Map as MapIcon, LogOut, Loader2 } from 'lucide-react';

/**
 * TravelView: คอมโพเนนต์แสดงผลหน้าจอการเดินทางและดันเจี้ยน
 * อัปเดต: ปรับธีมเป็นโทน Orange-Amber ให้เข้ากับระบบฉายาและ Points Available
 */
export default function TravelView({ 
  ...
  inDungeon, 
  onExitDungeon,
  isWalking,     // ✅ รับสถานะการเดินจาก App.jsx
  walkProgress,
  player   // ✅ รับเปอร์เซ็นต์ความคืบหน้า (0-100) จาก App.jsx
}) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {/* ==========================================
          ...
          [อัปเดต] เปลี่ยนธีมสีพื้นหลังเป็นโทนส้ม-ดำ (Orange-Amber Theme)
          ========================================== */}
      <div className={`
      ...
      `}>
        ...
        {/* Badge บอกสถานที่ปัจจุบัน (ปรับสีไอคอนและข้อความให้เข้ากับธีมส้ม) */}
        <div className="absolute top-6 left-6 flex items-center gap-2 px-3 py-1 bg-black/40 rounded-full border border-white/10 backdrop-blur-sm">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-orange-400/80">
            {isDungeonMode ? inDungeon.name : "แผนที่โลก"}
          </span>
        </div>

        {/* ==========================================
            ✨ เพิ่มตรงนี้: บันทึกระยะทางสะสม (ฝั่งขวา)
           ========================================== */}
        ...
        {/* เนื้อหาหลัก: เหตุการณ์ปัจจุบัน หรือ สถานะการบุกตะลุย (ปรับสี Font เป็นส้ม-เหลือง) */}
        <div className="text-center py-10">
  {currentEvent ? (
    <div className="animate-in fade-in zoom-in duration-500 flex flex-col items-center">
      {/* 🐾 ส่วนที่เพิ่มใหม่: แสดง Icon จาก Object */}
      {currentEvent.Icon && (
      ...
      <p className="text-orange-100/80 text-xl italic leading-relaxed">
              ...
              {isDungeonMode ? (
                <div className="space-y-4 flex flex-col items-center justify-center">
                    <p className="text-3xl text-orange-500 font-black uppercase tracking-tighter italic">
                      ...
                      {/* 🛠️ แก้ไข: ลบคำสะกดผิด และจัดกึ่งกลางหลอด */}
                ...
                <div className="opacity-60">
                  ...
                  <p className="text-white-400/70 mt-2 italic text-sm font-medium tracking-wide">
                    {isWalking ? "เสียงฝีเท้าดังก้องไปทั่วบริเวณ..." : "เส้นทางเบื้องหน้าเต็มไปด้วยอันตราย..."}
                  </p>
        ...
        <div className="space-y-3">
          {/* 🔘 ปุ่มก้าวเดินหลัก (พร้อมระบบ Progress Bar ภายในปุ่ม) */}
          <button 
            onClick={onStep}
            disabled={isWalking} // ✅ ปิดการใช้งานขณะกำลังโหลดเดิน
            ...
            `}
          >
            {/* 🟠 Progress Bar Overlay: วิ่งอยู่หลังตัวหนังสือ (ใช้สีส้ม Amber) */}
            {isWalking && (
              <div 
            ...
            <div className="relative z-10 flex items-center justify-center gap-3">
                ...
                <>
            {/* 🟠 เปลี่ยน text-white เป็น text-orange-500 ให้เหมือนกัน */}
             <span className="text-orange-500">
             {isDungeonMode ? "บุกเข้าไปต่อ!" : "ก้าวเดินต่อไป"}
              </span>
            {/* 🟠 เปลี่ยน text-red-500 เป็น text-orange-500 และลบอนิเมชั่น ping ออกเพื่อให้ดูนิ่งขึ้น หรือจะเก็บไว้ก็ได้ครับ */}
          ...
          {/* ✅ ปุ่มหลบหนีออกจากดันเจี้ยน */}
          {isDungeonMode && (
            <button 
              onClick={onExitDungeon} 
              disabled={isWalking} // ✅ ป้องกันการกดหนีขณะหลอดโหลดเดินกำลังวิ่ง
      ...
      {/* ==========================================
          📜 3. LOGS: ส่วนบันทึก Adventure Logs (ปรับสี Bullet ให้เป็นโทนส้ม-เหลือง)
          ========================================== */}
  ...
  {logs.map((log, i) => {
    // 🔍 ฟังก์ชันช่วยแยกข้อความเพื่อดึงตัวเลขโบนัสออกมา
    const parts = log.split(/([+-]\d+)/); // แยกตรงส่วนที่เป็น + ตามด้วยตัวเลข

    return (
      <div key={i} className="text-orange-100/60 text-sm flex gap-3 animate-in slide-in-from-left duration-300">
        {/* หัวลูกศร: แดงถ้าเจอศัตรู, เขียวถ้าฟื้นฟู, เหลืองถ้าทั่วไป */}
        <span className={`
        ...
        <span className="leading-tight">
          {parts.map((part, index) => 
            part.startsWith('+') ? (
              // ✅ ถ้าเป็นตัวเลขโบนัส ให้ใส่สีเขียว Emerald และทำตัวหนา
