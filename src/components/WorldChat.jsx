...
export default function WorldChat({ player, isMobile, onNewMessage, unreadChatCount }) {
  ...
  // ✨ 2. ระบบนับจำนวนผู้เล่นออนไลน์ (คงเดิม)
  useEffect(() => {
  ...
  const sendMessage = (e) => {
    ...
    // 🕵️ ระบบ Admin Command (/dev )
    const canUseAdminCommand = player.name === 'DEV001' || player.name === 'Admin';
    if (text.startsWith('/dev ') && canUseAdminCommand) {
  ...
  // ✅ ประกาศตัวแปรนี้ไว้เพื่อใช้ใน UI (Placeholder)
  const isUserDeveloper = player.name === 'DEV001' || player.name === 'Admin';
    ...
    <div className={`flex flex-col bg-slate-900/90 backdrop-blur-md border border-slate-700 rounded-lg overflow-hidden shadow-2xl transition-all duration-300
      ...
      <div className="bg-slate-800/80 p-2 flex justify-between items-center border-b border-slate-700">
        <div className="flex flex-col leading-tight">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            ...
            <span className="text-[8px] font-black text-emerald-400/80 uppercase tracking-tighter">{onlineCount} Players Active</span>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2 text-[12px] md:text-sm custom-scrollbar bg-slate-950/20">
        {messages
          ...
          .map((msg, i) => {
            // ✅ แก้ไข: เช็คชื่อ Admin และ DEV001 เพื่อแสดงผลกรอบสีฟ้า
            const msgFromDev = msg.username === 'DEV001' || msg.username === 'Admin';
            return (
              <div key={i} className={`animate-in fade-in slide-in-from-left-2 w-full ${msgFromDev ? 'py-1' : ''}`}>
                {msgFromDev ? (
                  <div className="relative group w-full">
                    ...
                    <div className="relative bg-slate-900/90 border-l-4 border-cyan-500 rounded-r-xl p-2.5 shadow-lg">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[7px] bg-cyan-500 text-slate-950 px-2 py-0.5 rounded-full font-black tracking-widest uppercase">DEVELOPER</span>
      ...
      <form onSubmit={sendMessage} className="p-2 border-t border-slate-700 flex gap-2 bg-slate-900/80">
        <input 
          ...
          // ✅ เปลี่ยนจาก isDeveloper เป็น isUserDeveloper ให้ตรงกับที่ประกาศไว้
          placeholder={isUserDeveloper ? "พิมพ์ข้อความ... " : "พิมพ์ข้อความ..."}
